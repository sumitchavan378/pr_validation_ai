...
The LLM backend is **Google Gemini** (configured via `GEMINI_MODEL`, default `gemini-2.5-flash`) using an API key from Google AI Studio. Orchestration uses **CrewAI**.
...
|----------|---------|-------------|
| `GEMINI_MODEL` | `gemini-2.5-flash` | Model id for all crews |
